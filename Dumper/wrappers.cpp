...
std::string UE_UFunction::GetFunctionFlags() const {
	...
	if (flags == FUNC_None) {
		...
		if (flags & FUNC_Final) {//Drg modification: replaced "|" with "," in all these things
			result += "Final,";
		}
...
void UE_UPackage::GenerateBitPadding(std::vector<Member>& members, uint32 offset, uint8 bitOffset, uint8 size) {
	Member padding;
	padding.Type = "padding";//DRG modification: changed type from "char" to "padding" to help filter these out from file
...
void UE_UPackage::GenerateStruct(UE_UStruct object, std::vector<Struct>& arr, bool findPointers) {
	...
	s.CppName = "UCLASS()\nclass " + object.GetCppName();//DRG modification: added UCLASS() thingy so that BP can see it I think

	auto super = object.GetSuper();
	if (super) {
		s.CppName += " : public " + super.GetCppName();//DRG modification: added public. I don't remember why but it's important
...
void UE_UPackage::SaveStruct(std::vector<Struct>& arr, FILE* file) {
	for (auto& s : arr) {
		...
		fmt::print(file, "\n\tGENERATED_BODY()\npublic:");//DRG modification: add in the generated body macro n stuff n whatever
		for (auto& m : s.Members) {//DRG modification: added ifs to handle padding and structs and delegates (oh my!) differently
			if (m.Type == "padding")
			{
				fmt::print(file, "\n\t//{} {}; // {:#04x}({:#04x})", m.Type, m.Name, m.Offset, m.Size);//just comment out the padding chars
			}
			else if (!(m.Type.find("Delegate") == std::string::npos))//if type contains delegate
			{
				auto outString = fmt::format("\n\tDECLARE_DYNAMIC_MULTICAST_DELEGATE(F{});\n\tUPROPERTY(BlueprintAssignable) F{} {};", m.Name, m.Name, m.Name);//A lil bit clumsy to declare a dynamic multicast every time. But I still got no idea what's up with inlines
				fmt::print(file, outString);
			}
			else
			{
				auto outString = fmt::format("\n\tUPROPERTY(BlueprintReadWrite) {} {}; // {:#04x}({:#04x})", m.Type, m.Name, m.Offset, m.Size);//add UPROPERTY to structs. Note that BLueprintReadWrite is hard-coded, since this tool doesn't grab the flags for properties and I aint smart enough to add in that feature
				while (!(outString.find("struct ") == std::string::npos))//yank out the "struct " instances, since they're supposed to be object references most of the time I think maybe I dunno man
				{
					auto structLoc = outString.find("struct ");
					outString.replace(structLoc, 7, "");
				}
				fmt::print(file, outString);
			}
		}
		if (s.Functions.size()) {
			...
			for (auto& f : s.Functions) {
				auto outString = fmt::format("\n\tUFUNCTION({}) {}({}); // {} // ({}) // @ game+{:#08x}", f.Flags, f.CppName, f.Params, f.FullName, f.Flags, f.Func - Base); //DRG modication: Added UFUNCTION as well as appropriate flags to this string
				while (!(outString.find("struct ") == std::string::npos))//yank out the "struct " instances, since they're supposed to be object references most of the time I think maybe I dunno man
				{
					auto structLoc = outString.find("struct ");
					outString.replace(structLoc, 7, "");
